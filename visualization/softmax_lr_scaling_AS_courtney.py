...
parser.add_argument('--optimizer', type=str, required=True, choices=['AS', 'AW'],
                    help='Optimizer type: AS (Adam-Star) or AW (AdamW)')
...
optimizer_map = {'AS': 'adamstar', 'AW': 'adamw'}
...
if __name__ == '__main__':
    ...
    if args.fit_compute and A_fit3 is not None:
        ...
        if len(compute_to_nonemb) >= 2:
            ...
            ax.plot(nonemb_extended, lr_fit3, '-.', color='tab:green', linewidth=3,
                    label=f'Fit 2 (Compute): {A_fit3:.2e} × $C^{{{B_fit3:.3f}}}$', zorder=10)
        else:
    ...
    if args.output:
        ...
        optimizer_filename_map = {'AS': 'Adam-Star', 'AW': 'AdamW'}
        optimizer_name = optimizer_filename_map[args.optimizer]
        output_file = f'softmax_lr_scaling_{optimizer_name}.pdf'
