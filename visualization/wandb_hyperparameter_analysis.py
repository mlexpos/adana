...
ADDITIONAL_FILTERS = {
    ...
    "config.opt": "snoo-dana",
    #"config.gamma_3_factor": 1.0,
    "config.iterations": 13953, #21481,
    "config.k": 20,
...
HYPERPARAMS_TO_ANALYZE = [
    ...
    "lr_outer",     # Outer learning rate
    "k",            # Nesterov step interval
...
def create_hyperparameter_visualization(df: pd.DataFrame, best_runs: pd.DataFrame):
    ...
    for i, param in enumerate(all_params):
        ...
        if param_col in df.columns:
            ...
            # Set y-axis upper limit to 4 for better readability
            #ax.set_ylim(top=3.9)
            #ax.set_ylim(bottom=3.8)
            
            # Use log scale for parameters that should be on log scale
            log_scale_params = ['lr', 'muon_lr_factor', 'renorm_weight_decay', 'weight_decay']
            if param in log_scale_params and df[param_col].min() > 0:
                ax.set_xscale('log')
