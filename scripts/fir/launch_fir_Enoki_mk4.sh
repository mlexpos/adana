...
# Enoki DanaStar-MK4 ScaledGPT Initialization Sweep for Fir
...
LR_MULTIPLIERS=( 1.0 )
CLIPSNR=2.0
# for dana-star-mk4: batch/accum depend on HEADS (set per-job later)
...
for OMEGA in "${OMEGA_ARRAY[@]}"; do
    ...
    for HEADS in "${HEADS_ARRAY[@]}"; do
        ...
        # Set batch size and accumulation steps per HEADS for dana-star-mk4
        if [ "$HEADS" -le 32 ]; then
            BATCH_SIZE=32
            ACC_STEPS=1
