...
# Enoki AdamW Standard Initialization 4-GPU Sweep for Narval
# Uses Standard initialization scheme with 4 GPUs
...
OMEGA_ARRAY=( 4.0 )
HEADS_ARRAY=( 18 20 )
LR_MULTIPLIERS=( 8.0 10.0 6.0 12.0 4.0 )

BATCH_SIZE=8   # Local batch size per GPU (need to be divisible by 4)
ACC_STEPS=4    # BATCH_SIZE * ACC_STEPS = 32 (global effective batch size)
...
echo "Starting Enoki AdamW Standard Initialization 4-GPU sweep (Narval)"
...
for OMEGA in "${OMEGA_ARRAY[@]}"; do
    ...
    for HEADS in "${HEADS_ARRAY[@]}"; do
        ...
        for MULT in "${LR_MULTIPLIERS[@]}"; do
            ...
            # Submit the job with 4-GPU configuration and Standard initialization
            sbatch --time=${TIME_HOURS}:00:00 \
                   --nodes=1 \
                   --gpus-per-node=a100:${GPUS_PER_NODE} \
                   --cpus-per-gpu=${CPUS_PER_GPU} \
                   --mem=${MEM} \
                   --job-name=EN_AW_STD4G_om${OMEGA}_h${HEADS}_lr${MULT} \
                   scripts/narval/Enoki_epaq_std.sh \
                   --heads $HEADS \
                   --lr $LR \
                   --omega $OMEGA \
