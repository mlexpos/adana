...
# Enoki AdamW ScaledGPT Initialization Sweep for Narval (no QK norm)
# Uses ScaledGPT initialization scheme with 1 A100 GPU and --no-qknorm flag
...
echo "Starting Enoki AdamW ScaledGPT Initialization sweep (Narval, no QK norm)"
...
for OMEGA in "${OMEGA_ARRAY[@]}"; do
    ...
    for HEADS in "${HEADS_ARRAY[@]}"; do
        ...
        for MULT in "${LR_MULTIPLIERS[@]}"; do
            ...
            # Submit the job with ScaledGPT initialization and --no-qknorm
            sbatch --time=${TIME_HOURS}:00:00 \
                   --nodes=1 \
                   --gpus-per-node=a100:${GPUS_PER_NODE} \
                   --cpus-per-gpu=${CPUS_PER_GPU} \
                   --mem=${MEM} \
                   --job-name=EN_AdamW_noQK_om${OMEGA}_h${HEADS}_lr${MULT} \
...
echo "  Init scheme: ${INIT_SCHEME}"
echo "  Depth scalar exponent: ${DEPTH_SCALAR_EXPONENT}"
echo "  WandB group: Enoki_ScaledGPT_noqk"
echo ""
