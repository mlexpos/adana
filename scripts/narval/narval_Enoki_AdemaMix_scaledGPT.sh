...
# Enoki AdemaMix ScaledGPT Initialization Single-GPU Sweep for Narval
...
# Learning rate formula: lr = 5.89e+01 * (1.12e04 + P)^{-0.644} where P = NON_EMB
# Enoki scaling: head_dim=64 (fixed), n_layer=3*heads/4, n_embd=64*heads, mlp=4*n_embd

OMEGA_ARRAY=( 4.0 )
HEADS_ARRAY=( 6 8 10 12 14 )
LR_MULTIPLIERS=( 1.0 1.25 0.75 1.5 0.25 )
...
echo "Starting Enoki AdemaMix ScaledGPT Initialization sweep (Narval)"
...
for OMEGA in "${OMEGA_ARRAY[@]}"; do
    ...
    for HEADS in "${HEADS_ARRAY[@]}"; do
        ...
        # Calculate base learning rate using formula: lr = 5.89e+01 * (1.12e04 + P)^{-0.644}
        BASE_LR=$(python3 -c "print(5.89e+01 * ((1.12e04 + $NON_EMB) ** -0.644))")
        ...
        for MULT in "${LR_MULTIPLIERS[@]}"; do
            ...
            sbatch --time=${TIME_HOURS}:00:00 \
                   ...
                   --mem=${MEM} \
                   --job-name=EN_AM_SGPT_om${OMEGA}_h${HEADS}_lr${MULT} \
