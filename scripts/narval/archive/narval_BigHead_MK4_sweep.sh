...
# BigHead MK4 Sweep across depths {4,5,6,7}
# For each depth, runs 3 learning rates: 0.75x, 1.0x, and 1.25x the formula prediction
# Learning rate formula: lr = 6.0e+04 × P^(-1.07) where P = NON_EMB
...
for DEPTH in "${DEPTHS[@]}"; do
    ...
    # Time estimation commented out as it wasn't useful
    # TIME_HOURS=$(python3 -c "import math; print(max(1, int(math.ceil(4.0 * $C / $C_4))))")
    TIME_HOURS=4

    # Calculate base learning rate using formula: lr = 6.0e+04 * P^(-1.07)
    BASE_LR=$(python3 -c "print(6.0e+04 * ($NON_EMB ** -1.07))")
    ...
    for MULT in "${LR_MULTIPLIERS[@]}"; do
        ...
        sbatch --time=${TIME_HOURS}:00:00 \
               --job-name=BH_MK4_d${DEPTH}_lr${MULT} \
