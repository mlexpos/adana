...
# Hoyer Loss Sweep for Narval
# Sweeps over hoyer_loss_coeff parameter with QK normalization disabled
...
# Hoyer loss coefficients to sweep
HOYER_COEFFS=(1e-2 1e-3 1e-4)
...
# QK normalization flag (DISABLED for this sweep)
NO_QKNORM=true

echo "Starting Hoyer Loss Coefficient Sweep (Narval)"
echo "=============================================="
echo ""
echo "Architectures:"
...
echo ""
echo "Hoyer loss coefficients: ${HOYER_COEFFS[@]}"
echo ""
...
# Sweep over Hoyer loss coefficients
for HOYER_COEFF in "${HOYER_COEFFS[@]}"; do
    job_count=$((job_count + 1))
    echo "Job $job_count: Enoki with hoyer_loss_coeff=$HOYER_COEFF"
    ...
    sbatch --time=${TIME_HOURS}:00:00 \
           ...
           --mem=${MEM} \
           --job-name=Hoyer_Enoki_h${ENOKI_HEADS}_${HOYER_COEFF} \
...
for HOYER_COEFF in "${HOYER_COEFFS[@]}"; do
    job_count=$((job_count + 1))
    echo "Job $job_count: Qwen3 with hoyer_loss_coeff=$HOYER_COEFF"
    ...
    sbatch --time=${TIME_HOURS}:00:00 \
           ...
           --cpus-per-gpu=${CPUS_PER_GPU} \
           --mem=${MEM} \
           --job-name=Hoyer_Qwen3_h${QWEN3_HEADS}_${HOYER_COEFF} \
...
echo ""
echo "Jobs per architecture: ${#HOYER_COEFFS[@]}"
echo ""
