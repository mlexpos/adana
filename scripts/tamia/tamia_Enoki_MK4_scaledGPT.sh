...
# Enoki DanaStar-MK4 ScaledGPT Initialization Multi-GPU Sweep for Tamia
...
# Learning rate formula: lr = 9.26e+03 * (1.04e04 + P)^{-0.966} where P = NON_EMB
# Enoki scaling: head_dim=64 (fixed), n_layer=3*heads/4, n_embd=64*heads, mlp=4*n_embd

OMEGA_ARRAY=( 4.0 )
HEADS_ARRAY=( 22 24 )
LR_MULTIPLIERS=( 1.75 2.0 2.25)
CLIPSNR=2.0
...
for OMEGA in "${OMEGA_ARRAY[@]}"; do
    ...
    for HEADS in "${HEADS_ARRAY[@]}"; do
        ...
        # Calculate base learning rate using formula: lr = 9.26e+03 * (1.04e04 + P)^{-0.966}
        BASE_LR=$(python3 -c "print(9.26e+03 * ((1.04e04 + $NON_EMB) ** -0.966))")
