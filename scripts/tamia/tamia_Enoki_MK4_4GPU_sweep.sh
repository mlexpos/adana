...
# Enoki MK4 Multi-GPU Sweep for Tamia
...
# Learning rate formula: lr = 1.35e-06 + 9.30e+03 × P^{-0.967} where P = NON_EMB
# Enoki scaling: head_dim=64 (fixed), n_layer=3*heads/4, n_embd=64*heads, mlp=4*n_embd

OMEGA=4.0
CLIPSNR=2.0
HEADS_ARRAY=( 24 )
LR_MULTIPLIERS=(0.75 1.5)
...
echo "Starting Enoki MK4 Multi-GPU sweep (Tamia)"
...
for HEADS in "${HEADS_ARRAY[@]}"; do
    ...
    # Calculate base learning rate using formula: lr = 1.35e-06 + 9.30e+03 × P^{-0.967}
    BASE_LR=$(python3 -c "print(1.35e-06 + 9.30e+03 * ($NON_EMB ** -0.967))")
