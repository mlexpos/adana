...
# Learning rate formula: lr = 2.761429e+04 × C^-0.4320 where compute = iterations * non_emb * 6 * 2048 * 32
...
for DEPTH in "${DEPTHS[@]}"; do
    ...
    if [ $DEPTH -le 6 ]; then
        TIME_SPEC="00:30:00"
    elif [ $DEPTH -le 9 ]; then
        TIME_SPEC="03:00:00"
    elif [ $DEPTH -le 11 ]; then
        TIME_SPEC="12:00:00"
    ...
    BASE_LR=$(python3 -c "print(2.761429e+04 * ($COMPUTE ** -0.4320))")
    ...
    for MULT in "${LR_MULTIPLIERS[@]}"; do
        ...
        sbatch --time=${TIME_SPEC} \
               ...
               scripts/scripts_dfer/fir_BigHead_dfer.sh \
               --depth $DEPTH \
               --lr $LR \
               --omega $OMEGA \
               --optimizer ademamix \
