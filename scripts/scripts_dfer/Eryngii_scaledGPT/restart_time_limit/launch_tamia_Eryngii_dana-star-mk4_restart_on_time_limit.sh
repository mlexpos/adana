...
echo "Starting Eryngii AdamW Multi-GPU sweep (Dana-Star-MK4) with restart on time limit"
echo "Heads: ${HEADS[@]}"
echo "Omega: $OMEGA"
...
for HEADS in "${HEADS[@]}"; do
    ...
    for MULT in "${LR_MULTIPLIERS[@]}"; do
        ...
        sbatch --time=${TIME_SPEC} \
               ...
               scripts/scripts_dfer/Eryngii_scaledGPT/restart_time_limit/tamia_Eryngii_dfer.sh \
               --heads $HEADS \
               --lr $LR \
               --omega $OMEGA \
