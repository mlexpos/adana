...
# Enoki Ademamix ScaledGPT Initialization Single-GPU Sweep for Tamia
...
# Learning rate formula: lr = 4.51 × (6.71e+03 + P)^-0.503 where P = NON_EMB
...
# SLURM configuration for Tamia (4 GPUs)
GPUS_PER_NODE=4
CPUS_PER_GPU=8
...
echo "Starting Enoki Ademamix ScaledGPT Initialization sweep (Tamia)"
...
for OMEGA in "${OMEGA_ARRAY[@]}"; do
    ...
    for HEADS in "${HEADS_ARRAY[@]}"; do
        ...
        BASE_LR=$(python3 -c "print(4.51 * ((6.71e03 + $NON_EMB) ** -0.503))")
        ...
        # Set batch size and accumulation steps per HEADS for adamw
        if [ "$HEADS" -le 32 ]; then
            BATCH_SIZE=32
            ACC_STEPS=1
        ...
        for MULT in "${LR_MULTIPLIERS[@]}"; do
            ...
            sbatch --account=aip-gidelgau \
                   ...
                   --mem=${MEM} \
                   --job-name=EN_Ademamix_SGPT_om${OMEGA}_h${HEADS}_lr${MULT} \
                   scripts/scripts_dfer/Enoki_scaledGPT_restart/tamia_Enoki_dfer.sh \
                   --heads $HEADS \
                   --lr $LR \
                   --omega $OMEGA \
