...
# Enoki Dana-Star-No-Tau ScaledGPT Initialization Single-GPU Sweep for Fir
...
# Learning rate formula: 1.34e+01 \times (7.04e03 + P)^{-0.627} where P = NON_EMB
...
echo "Starting Enoki DanaStar-No-Tau ScaledGPT Initialization sweep (Fir)"
...
for OMEGA in "${OMEGA_ARRAY[@]}"; do
    ...
    for HEADS in "${HEADS_ARRAY[@]}"; do
        ...
        BASE_LR=$(python3 -c "print(1.34e01 * ((7.04e03 + $NON_EMB) ** -0.627))")
        ...
        for MULT in "${LR_MULTIPLIERS[@]}"; do
            ...
            sbatch --account=rrg-bengioy-ad \
                   ...
                   --mem=${MEM} \
                   --job-name=EN_DSNT_SGPT_om${OMEGA}_h${HEADS}_lr${MULT} \
                   scripts/scripts_dfer/Enoki_scaledGPT_restart/fir_Enoki_dfer.sh \
                   --heads $HEADS \
                   --lr $LR \
                   --omega $OMEGA \
