...
# Enoki D-Muon ScaledGPT Initialization Single-GPU Sweep for Nibi
...
# Learning rate formula: lr = 2.19 * (5.64e4 + P)^-0.417 where P = NON_EMB
# Enoki scaling: head_dim=64 (fixed), n_layer=3*heads/4, n_embd=64*heads, mlp=4*n_embd
# iterations to run formula = 24*3600 / (6.16 * 10^(-4) * (Total_params/1e6)^0.94) / 2 *(GPUs_per_node/4)
...
# SLURM configuration for Nibi (8 GPUs per node)
GPUS_PER_NODE=8
CPUS_PER_GPU=8
TOTAL_CPUS=64            # 8 GPUs × 8 CPUs/GPU
...
echo "Starting Enoki D-Muon ScaledGPT Initialization sweep (Nibi)"
...
for OMEGA in "${OMEGA_ARRAY[@]}"; do
    ...
    for HEADS in "${HEADS_ARRAY[@]}"; do
        ...
        # Calculate base learning rate using formula: lr = 4.51 * (6.71e+03 + P)^{-0.503}
        ...
        for MULT in "${LR_MULTIPLIERS[@]}"; do
            ...
            ITERATIONS_TO_RUN=$(python3 -c "print(int(24 * 3600 / (6.16e-04 * ($TOTAL_PARAMS/1e6) ** 0.94) / 2 * ($GPUS_PER_NODE/4)))")
            ...
            sbatch --account=rrg-bengioy-ad \
                   ...
                   scripts/scripts_dfer/Enoki_scaledGPT_restart/nibi_Enoki_dfer.sh \
                   --heads $HEADS \
                   --lr $LR \
                   --omega $OMEGA \
