...
# Learning rate formula: 4.40e+01 × (8.35e3 + P)^-0.664 where P = NON_EMB
...
echo "Starting Enoki DanaStar-MK4 ScaledGPT Initialization sweep (Tamia)"
...
for OMEGA in "${OMEGA_ARRAY[@]}"; do
    ...
    for HEADS in "${HEADS_ARRAY[@]}"; do
        ...
        BASE_LR=$(python3 -c "print(4.40e01 * ((8.35e03 + $NON_EMB) ** -0.664))")
        ...
        for MULT in "${LR_MULTIPLIERS[@]}"; do
            ...
            sbatch --account=aip-gidelgau \
                   ...
                   --mem=${MEM} \
                   --job-name=EN_MK4_SGPT_om${OMEGA}_h${HEADS}_lr${MULT} \
