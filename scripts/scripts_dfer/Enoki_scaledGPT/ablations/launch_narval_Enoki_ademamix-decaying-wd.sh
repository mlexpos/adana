...
# Learning rate formula: lr = 1.70e+02 × (1.00e + 04 + P)^-0.695 where P = NON_EMB
# Enoki scaling: head_dim=64 (fixed), n_layer=3*heads/4, n_embd=64*heads, mlp=4*n_embd

OMEGA_ARRAY=( 4.0 )
HEADS_ARRAY=(8)
#HEADS_ARRAY=( 6 )
LR_MULTIPLIERS=( 1.0 1.25 1.5 0.75)
#LR_MULTIPLIERS=( 1.0 )
...
for OMEGA in "${OMEGA_ARRAY[@]}"; do
    ...
    for HEADS in "${HEADS_ARRAY[@]}"; do
        ...
        # Calculate base learning rate using formula: lr = 1.70e+02 × (1.00e + 04 + P)^-0.695
        BASE_LR=$(python3 -c "print(1.70e+02 * (1.00e+04 + $NON_EMB) ** -0.695)")
