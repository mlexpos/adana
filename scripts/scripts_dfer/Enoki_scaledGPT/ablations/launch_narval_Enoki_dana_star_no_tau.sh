...
# Enoki Dana-Star-No-Tau ScaledGPT Initialization Single-GPU Sweep for Narval
...
# Learning rate formula: lr = 1.27e+01 / 2 × (1.99e + 05 + P)^-0.582 where P = NON_EMB
# Enoki scaling: head_dim=64 (fixed), n_layer=3*heads/4, n_embd=64*heads, mlp=4*n_embd

OMEGA_ARRAY=( 4.0 )
HEADS_ARRAY=( 6 8 10 12 14 16 18)
#HEADS_ARRAY=( 6 )
LR_MULTIPLIERS=( 1.0 1.25 1.5 0.75 0.5)
#LR_MULTIPLIERS=( 1.0 )

BATCH_SIZE=32
ACC_STEPS=1

# SLURM configuration for Narval (1 GPU per node)
GPUS_PER_NODE=1
CPUS_PER_GPU=4
TOTAL_CPUS=4
...
echo "Starting Enoki Dana-Star-No-Tau ScaledGPT Initialization sweep (Narval)"
...
for OMEGA in "${OMEGA_ARRAY[@]}"; do
    ...
    for HEADS in "${HEADS_ARRAY[@]}"; do
        ...
        # Calculate base learning rate using formula: lr = 1.27e+01 / 2 × (1.99e + 05 + P)^-0.582
        BASE_LR=$(python3 -c "print(1.27e+01 / 2 * (1.99e+05 + $NON_EMB) ** -0.582)")
        ...
        for MULT in "${LR_MULTIPLIERS[@]}"; do
            ...
            sbatch --time=${TIME_HOURS}:00:00 \
                   ...
                   --mem=${MEM} \
                   --job-name=EN_Dana-Star-No-Tau_SGPT_om${OMEGA}_h${HEADS}_lr${MULT} \
