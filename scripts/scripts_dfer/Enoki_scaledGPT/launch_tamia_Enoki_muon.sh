...
# Enoki D-Muon ScaledGPT Initialization Single-GPU Sweep for Fir
...
# Learning rate formula: lr = 2.67e+02 × (2.96e7 + P)^-0.664 where P = NON_EMB
...
echo "Starting Enoki D-Muon ScaledGPT Initialization sweep (Fir)"
...
for OMEGA in "${OMEGA_ARRAY[@]}"; do
    ...
    for HEADS in "${HEADS_ARRAY[@]}"; do
        ...
        # Calculate base learning rate using formula: lr = 2.67e+02 × (2.96e7 + P)^-0.664
        BASE_LR=$(python3 -c "print(2.67e+02 * (2.96e7 + $NON_EMB) ** -0.664)")
        ...
        for MULT in "${LR_MULTIPLIERS[@]}"; do
            ...
            sbatch --time=${TIME_HOURS}:00:00 \
                   ...
                   scripts/scripts_dfer/Enoki_scaledGPT/tamia_Enoki_dfer.sh \
                   --heads $HEADS \
                   --lr $LR \
                   --omega $OMEGA \
