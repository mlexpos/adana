...
# Enoki Dana ScaledGPT Initialization Single-GPU Sweep for Fir
...
OMEGA_ARRAY=( 4.0 )
# HEADS_ARRAY=( 6 8 )
# GAMMA_3_FACTOR_POWERS=( -0.25 -0.5 -0.75 -1.0 -1.5 -2.0 -4.0 )
# ITERATION_FACTOR_ARRAY=( 0.1 0.3 1.0 3.0 9.0 )
HEADS_ARRAY=(6)
...
for OMEGA in "${OMEGA_ARRAY[@]}"; do
    ...
    for HEADS in "${HEADS_ARRAY[@]}"; do
        ...
        for ITERATION_FACTOR in "${ITERATION_FACTOR_ARRAY[@]}"; do
            ...
            ITERATIONS=$(python3 -c "print($ITERATIONS * $ITERATION_FACTOR)")
            GAMMA_3_FACTOR=$(python3 -c "print($ITERATIONS ** $GAMMA_3_FACTOR_POWER)")
            ...
            sbatch --time=${TIME_HOURS}:00:00 \
                   ...
                   scripts/scripts_dfer/Enoki_scaledGPT/gamma_3_search/fir_Enoki_dfer_gamma3_search.sh \
                   --heads $HEADS \
                   --lr $LR \
                   --omega $OMEGA \
