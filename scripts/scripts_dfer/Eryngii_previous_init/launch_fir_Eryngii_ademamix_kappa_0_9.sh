...
# Eryngii Ademamix Multi-GPU Sweep for Fir (heads 4 to 12)
# Uses 4 GPUs per node for larger models
# For each n_head value, runs multiple learning rates: multipliers of base LR
# Base learning rate: lr = 1.29e-04×C -0.399

OMEGA=4.0
HEADS=(4 5 6 7 8 9 10 11 12)
...
for HEADS in "${HEADS[@]}"; do
    ...
    # Base learning rate
    BASE_LR=$(python3 -c "print(1.29e-04 * $C**(-0.399))")

    echo "  NON_EMB = $NON_EMB"
    echo "  ITERATIONS = $ITERATIONS"
    echo "  C = $(python3 -c "print($C)") PetaFLOPS"
