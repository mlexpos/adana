...
# Enoki DANA-MK4 ScaledGPT Initialization Sweep for Rorqual
...
# Learning rate formula: lr = 5.60e+01 × (5.71e+03 + P)^-0.708 where P = NON_EMB
...
echo "Starting Enoki DANA-MK4 ScaledGPT Initialization sweep (Rorqual)"
...
for OMEGA in "${OMEGA_ARRAY[@]}"; do
    ...
    for HEADS in "${HEADS_ARRAY[@]}"; do
        ...
        # Calculate base learning rate using formula: lr = 5.60e+01 × (5.71e+03 + P)^-0.708
        BASE_LR=$(python3 -c "print(5.60e+01 * ((5.71e03 + $NON_EMB) ** -0.708))")
  ...
  #          ITERATIONS_TO_RUN=$(python3 -c "print(int(24 * 3600 / (5.83e-04 * ($TOTAL_PARAMS/1e6) ** 0.91) / 2))")
            ...
            sbatch --account=rrg-bengioy-ad \
                   ...
                   --mem=${MEM} \
                   --job-name=EN_DAMK4_SGPT_om${OMEGA}_h${HEADS}_lr${MULT} \
                   scripts/rorqual/rorqual_Enoki_dana-mk4.sh \
                   --heads $HEADS \
                   --lr $LR \
                   --omega $OMEGA \
...
echo "  Head counts: ${HEADS_ARRAY[@]}"
echo "  LR multipliers: ${LR_MULTIPLIERS[@]}"
echo "  LR formula: lr = 5.60e+01 × (5.71e+03 + NON_EMB)^-0.708"
echo "  Clip SNR: $CLIPSNR"
echo ""
