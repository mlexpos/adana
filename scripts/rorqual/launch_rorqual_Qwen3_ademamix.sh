...
# Qwen3 AdeMaMix ScaledGPT Initialization Sweep for Rorqual
...
echo "Starting Qwen3 AdeMaMix ScaledGPT Initialization sweep (Rorqual)"
...
for OMEGA in "${OMEGA_ARRAY[@]}"; do
    ...
    for HEADS in "${HEADS_ARRAY[@]}"; do
        ...
        BASE_LR=$(python3 -c "print(4.51 * ((6.71e+03 + $NON_EMB) ** -0.503))")
        ...
        for MULT in "${LR_MULTIPLIERS[@]}"; do
            ...
            sbatch --account=rrg-bengioy-ad \
                   ...
                   scripts/rorqual/rorqual_Qwen3_ademamix.sh \
                   --heads $HEADS \
                   --lr $LR \
                   --omega $OMEGA \
...
echo "  Head counts: ${HEADS_ARRAY[@]}"
echo "  LR multipliers: ${LR_MULTIPLIERS[@]}"
echo "  LR formula: lr = 4.51 × (6.71e+03 + NON_EMB)^-0.503"
echo "  Clip SNR: $CLIPSNR"
echo ""
