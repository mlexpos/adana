...
# Qwen3 Dana-MK4 ScaledGPT Initialization Sweep for Rorqual
...
echo "Starting Qwen3 Dana-MK4 ScaledGPT Initialization sweep (Rorqual)"
...
for OMEGA in "${OMEGA_ARRAY[@]}"; do
    ...
    for HEADS in "${HEADS_ARRAY[@]}"; do
        ...
        BASE_LR=$(python3 -c "print(4.40e+01 * ((8.35e+03 + $NON_EMB) ** -0.415))")
        ...
        for MULT in "${LR_MULTIPLIERS[@]}"; do
            ...
            sbatch --account=rrg-bengioy-ad \
                   ...
                   --mem=${MEM} \
                   --job-name=Q3_DanaMK4_SGPT_om${OMEGA}_h${HEADS}_lr${MULT} \
                   scripts/rorqual/rorqual_Qwen3_dana-mk4.sh \
                   --heads $HEADS \
                   --lr $LR \
                   --omega $OMEGA \
...
echo "  Head counts: ${HEADS_ARRAY[@]}"
echo "  LR multipliers: ${LR_MULTIPLIERS[@]}"
echo "  LR formula: lr = 4.40e+01 × (8.35e+03 + NON_EMB)^-0.415"
echo "  Clip SNR: $CLIPSNR"
echo ""
