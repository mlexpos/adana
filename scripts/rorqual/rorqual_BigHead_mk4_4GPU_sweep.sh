...
# BigHead MK4 Multi-GPU Sweep for Rorqual (depths {8,9})
# Uses 4 GPUs per node
# For each depth, runs multiple learning rates: 0.5x, 0.75x, 1.0x, 1.25x, and 1.5x the formula prediction
...
GPUS_PER_NODE=4
CPUS_PER_GPU=12
TOTAL_CPUS=48  # 4 GPUs × 8 CPUs/GPU
MEM=0          # 0 = allocate as needed
TIME_HOURS=24

echo "Starting BigHead MK4 Multi-GPU sweep (Rorqual)"
echo "Depths: ${DEPTHS[@]}"
echo "Omega: $OMEGA"
...
for DEPTH in "${DEPTHS[@]}"; do
    ...
    BASE_LR=$(python3 -c "print(1.84e-05 + 1.37e+03 * ($NON_EMB ** -0.880))")
    ...
    for MULT in "${LR_MULTIPLIERS[@]}"; do
        ...
        sbatch --time=${TIME_HOURS}:00:00 \
               ...
               scripts/rorqual/BigHead_rorqual.sh \
               --depth $DEPTH \
               --lr $LR \
               --omega $OMEGA \
