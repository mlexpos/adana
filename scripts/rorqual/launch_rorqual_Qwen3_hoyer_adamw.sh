...
# Qwen3 Hoyer Loss Learning Rate Sweep for Rorqual (AdamW)
...
# SLURM configuration for Rorqual (1 H100 GPU)
GPUS_PER_NODE=4
CPUS_PER_GPU=8
...
echo "Starting Qwen3 Hoyer Loss LR Sweep - AdamW (Rorqual)"
...
for OMEGA in "${OMEGA_ARRAY[@]}"; do
    ...
    for HEADS in "${HEADS_ARRAY[@]}"; do
        ...
        # Calculate base learning rate using AdamW formula: lr = 1.27e+04 × (9.23e+04 + P)^-0.848
        ...
        echo "    Time allocation: ${TIME_HOURS}h"
        echo "    Base LR (AdamW formula): $BASE_LR"
        ...
        for MULT in "${LR_MULTIPLIERS[@]}"; do
            ...
            echo "    Job $job_count/$total_jobs: omega=$OMEGA, heads=$HEADS, lr=$LR (${MULT}x base), hoyer=$HOYER_LOSS_COEFF, opt=adamw"
            ...
            sbatch --account=rrg-bengioy-ad \
                   ...
                   --mem=${MEM} \
                   --job-name=Q3_Hoyer_AdamW_om${OMEGA}_h${HEADS}_lr${MULT} \
                   scripts/rorqual/rorqual_Qwen3_hoyer.sh \
                   --heads $HEADS \
                   --lr $LR \
                   --omega $OMEGA \
                   --optimizer adamw \
...
echo "  Head counts: ${HEADS_ARRAY[@]}"
echo "  LR multipliers: ${LR_MULTIPLIERS[@]}"
echo "  LR formula: lr = 1.27e+04 × (9.23e+04 + NON_EMB)^-0.848 (AdamW)"
echo "  Hoyer loss coefficient: $HOYER_LOSS_COEFF (fixed)"
echo "  QK Normalization: DISABLED"
echo "  Tau stats collection: ENABLED"
echo ""
