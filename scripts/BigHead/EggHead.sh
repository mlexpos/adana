...
# Validate heads argument
if [ -z "$HEADS" ]; then
    echo "Error: --heads argument is required"
    echo "Usage: ./EggHead.sh --heads <heads>=4> [options]"
    echo "Options:"
...
# Calculate model parameters based on number of heads using Python for correct integer types
# Formula: head_dim = 16 * heads, n_embd = 16 * heads^2, mlp = 32 * heads^2
HEAD_DIM=$(python3 -c "print(int(16 * $HEADS))")
N_EMBD=$(python3 -c "print(int(16 * $HEADS * $HEADS))")
MLP_HIDDEN=$(python3 -c "print(int(32 * $HEADS * $HEADS))")
N_HEAD=$(python3 -c "print(int($HEADS))")
N_LAYER=$(python3 -c "print(int($HEADS * ($HEADS - 1) / 2))")
...
NON_EMB=$(python3 -c "print(int($N_LAYER * (3 * $HEAD_DIM * $N_EMBD * $N_HEAD + $N_EMBD * $N_EMBD + 2 * $N_EMBD * $MLP_HIDDEN + 8 * $N_EMBD) + 2 * $N_EMBD))")
...
echo "=== EggHead Configuration for $HEADS Heads ==="
