...
GATING=1  # Enable elementwise attention gating by default
...
if [ -z "$HEADS" ]; then
    ...
    echo "Usage: ./Qwen3.sh --heads <heads> [options]"
    echo ""
    echo "Qwen3 uses the following scaling (from Qwen3 paper):"
    echo "  - head_dim = 128 (fixed, as per Qwen3 paper)"
    echo "  - n_head = heads"
    ...
    echo "  --gating <0|1>            Enable elementwise attention output gating (default: 1)"
    exit 1
fi

if [ "$HEADS" -lt 1 ]; then
    echo "Error: heads must be at least 1"
    exit 1
fi
...
# Formula depends on whether gating is enabled
# With gating: non_emb = n_layer * (5 * n_embd * (n_head * head_dim) + 2 * head_dim + 9 * n_embd^2 + 2 * n_embd) + n_embd
# Without gating: non_emb = n_layer * (4 * n_embd * (n_head * head_dim) + 2 * head_dim + 9 * n_embd^2 + 2 * n_embd) + n_embd
#
# Breaking down per layer:
# - Attention (with gating): 5 * n_embd * total_qkv_dim (q_proj 2x + k_proj + v_proj + o_proj)
# - Attention (no gating): 4 * n_embd * total_qkv_dim
# - QK norms: 2 * head_dim
# - MLP (SwiGLU): 2 * n_embd * mlp_hidden + mlp_hidden * n_embd = 9 * n_embd^2 (since mlp_hidden = 3 * n_embd)
...
# Per layer
attn = 4 * n_embd * total_qkv_dim  # q_proj + k_proj + v_proj + o_proj
qk_norm = 2 * head_dim
mlp = 9 * n_embd * n_embd
layer_norms = 2 * n_embd
...
# Total params = non_emb + 2 * n_embd * vocab_size (token embeddings + lm_head)
...
echo "n_head: $N_HEAD"
echo "qkv_dim (head_dim): $HEAD_DIM (fixed, as per Qwen3 paper)"
echo "n_embd: $N_EMBD (= $HEADS * 128)"
echo "mlp_hidden_dim: $MLP_HIDDEN (= 3 * $N_EMBD)"
echo "Elementwise gating: $GATING"
echo "Non-embedding parameters: $NON_EMB"
...
# Build gating flag
if [ "$GATING" -eq 1 ]; then
    GATING_FLAG="--elementwise_attn_output_gate"
else
    GATING_FLAG=""
fi
