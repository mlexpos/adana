...
class DataParallelDistributedBackend(DistributedBackend):
    def __init__(self, args):
        ...
        self.backend = args.distributed_backend
        # Enforce CUDA only for NCCL; allow CPU for Gloo
        if self.backend == "nccl":
            assert "cuda" in args.device, "DDP nccl backend requires CUDA devices"
        init_process_group(backend=self.backend)
        self.local_rank = int(os.environ["LOCAL_RANK"])
        self._initial_device = args.device
    ...
    def get_adjusted_args_for_process(self, args):
        ...
        # Device selection depends on backend
        if self.backend == "nccl":
            args.device = f"cuda:{self.local_rank}"
        else:
            # For CPU DDP (gloo), keep CPU device
            args.device = "cpu"
    ...
    def transform_model(self, model):
        ...
        # CPU DDP (gloo) should not pass device_ids
        return DDP(model)
